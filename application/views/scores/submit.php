<h4>Search</h4>
<p>
    Click on a song title to submit a score for that file.
</p>
<form action="#" id="chart-search">
    <div class="row">
        <div class="large-3 columns">
            <label for="chart-name">Chart Name</label>
            <input type="text" id="chart-name" name="chart-name" />
        </div>
        <div class="large-3 columns">
        ...
        <div class="large-3 columns">
            ...
            <select name="pack-name" id="pack-name">
                <option value="">
                    Select a Pack
                </option>
                <?php foreach ($packs as $pack) : ?>
                    <option value="<?=$pack->name;?>">
                        <?=$pack->name;?> <?=(!empty($pack->abbreviation) ? "(" . $pack->abbreviation . ")" : "");?>
                    </option>
                <?php endforeach; ?>
            </select>
        </div>
    ...
    <tbody>
        <?php foreach ($songs as $song) : ?>
            <tr>
                ...
                <td>
                    <?=$song->pack_name;?> <?=(!empty($song->pack_abbr) ? "(" . $song->pack_abbr . ")" : "");?>
                </td>
                <td>
